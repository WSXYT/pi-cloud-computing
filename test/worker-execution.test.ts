...
test("prepares an uploaded repository and returns its remote changes", async () => {
  ...
  assert.equal(
    (await readFile(join(prepared.workspace, "file.txt"), "utf8")).replaceAll(
      "\r\n",
      "\n",
    ),
    "before\n",
  ...
  assert.equal(
    await readFile(join(prepared.runtimeAgentDir ?? "", "auth.json"), "utf8"),
    '{"provider":"secret"}',
  );
  ...
  const result = parseGitSnapshot(
    (await store.read(String(payload.resultArtifactId))).toString("utf8"),
  );
  assert.deepEqual(
